        ...
        {groups.map((g,gi)=>{
          ...
          return(
            <g key={gi} style={{cursor:'pointer'}} onClick={()=>setActive(isActive?null:g.key)}>
              ...
              <text x={gX+groupW/2} y={H+22} textAnchor="middle" fontSize="7" fontWeight={isActive?700:500} fill={isActive?'#1a1a1a':'#666'}>{g.label}</text>
            </g>
          );
        })}
        <g transform={`translate(${padL},${H+34})`}>
          <rect width="7" height="7" fill="#5b8db8" rx="1"/><text x="10" y="6" fontSize="7" fill="#888">Legitimate</text>
          <rect x="76" width="7" height="7" fill="#e8729a" rx="1"/><text x="86" y="6" fontSize="7" fill="#888">Fraud</text>
        </g>
      </svg>
...
function P6ParallelCases() {
  const [active, setActive] = useState(null);
  const cases = [
    { id:0, industry:'Finance · Global', color:'#5b8db8', title:'JP Morgan — Market Regime Detection', tag:'Same methodology, bigger scale', desc:'JP Morgan quant team uses Hurst exponent analysis — the same core signal this study uses.', ref:'JP Morgan Quantitative Research · jpmorgan.com', refUrl:'https://www.jpmorgan.com', analysis:'JP Morgan research uses multifractal analysis to characterize market regimes — precisely the MF-DFA approach in this study. Hurst H above 0.65 reliably precedes elevated volatility by 2 to 4 weeks. This study independently confirms that on the Vietnamese market.', approach:'MF-DFA regime detection and options market signals', relevance:'Core methodology validated by one of the largest quant research teams globally' },
    { id:1, industry:'RegTech · Global', color:'#5a9e82', title:'ESMA — Social Media Bubble Monitoring', tag:'Behavioral signal validation', desc:'European Securities and Markets Authority uses social media sentiment and search trends as early warning indicators for market surveillance.', ref:'European Securities Markets Authority · esma.europa.eu', refUrl:'https://www.esma.europa.eu', analysis:'ESMA Market Intelligence monitors retail investor sentiment via social media and search trends — the same behavioral signal this study uses via Google Trends SVI. Their GameStop report confirmed retail search momentum preceded price spikes by 5 to 10 days.', approach:'Social media sentiment monitoring and search trend surveillance', relevance:'Regulatory validation that behavioral signals are legitimate early warning tools' },
    { id:2, industry:'Finance · Vietnam', color:'#9060c0', title:'SSC Vietnam — Market Surveillance 2023', tag:'Direct local application', desc:'State Securities Commission identified retail investor over-concentration as the key systemic risk in the 2022 crash — exactly what Google Trends measured.', ref:'State Securities Commission · ssc.gov.vn', refUrl:'https://www.ssc.gov.vn', analysis:'The SSC post-mortem on the 2022 crash cited FOMO-driven retail participation as the primary bubble mechanism — the exact phenomenon Google Trends Delta-SVI captures. This study provides the SSC with a quantifiable real-time proxy for that FOMO.', approach:'Real-time market surveillance with intervention thresholds tied to objective data signals', relevance:'System designed to be implementable within SSC existing surveillance framework' },
  ];
  const activeCase = active!==null ? cases[active] : null;
  return (
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
...
<li style={{fontSize:14,color:'#3a3a3a',lineHeight:1.7,paddingLeft:16,position:'relative',textAlign:'justify'}}>
  ...
  <strong>Benchmark for extreme class imbalance:</strong> With a fraud rate of only 0.17%, Random Forest achieved <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>84% recall</span> with just <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>10 false positives</span> — concrete numbers for future studies to reference when testing other models under the same conditions.
</li>
<li style={{fontSize:14,color:'#3a3a3a',lineHeight:1.7,paddingLeft:16,position:'relative',textAlign:'justify'}}>
  <span style={{position:'absolute',left:0}}>•</span>
  <strong>Quantified impact of SMOTE and Robust Scaler:</strong> SMOTE improved recall from <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>63% to 94%</span> for Logistic Regression, and Robust Scaler successfully handled the <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>$25,691 outlier</span> — reusable and verifiable technical details.
</li>
<li style={{fontSize:14,color:'#3a3a3a',lineHeight:1.7,paddingLeft:16,position:'relative',textAlign:'justify'}}>
  <span style={{position:'absolute',left:0}}>•</span>
  <strong>Clarified trade-offs across classifiers:</strong> <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>Logistic Regression</span> (maximizes recall, accepts high false positives), <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>Decision Tree</span> (middle ground), and <span style={{background:'#eef5fb',color:'#3a7db8',fontWeight:700,padding:'0 3px',borderRadius:2}}>Random Forest</span> (best balance) — helping future studies select the right model for their risk threshold.
</li>
                                    </ul>
                                  </div>
                                </div>
                              ...
                              <div className={styles.panelContent}>
                                ...
                                <div className={styles.panelBlock}>
                                  ...
                                  <div className={styles.reflectGrid}>
                                    ...
                                    <div className={styles.reflectCard} style={{borderTopColor:'#9060c0'}}><div className={styles.reflectHeader} style={{color:'#9060c0'}}><span>→</span> What I would do differently</div><ul className={styles.reflectList}><li>Add <span className={styles.reflectHL}>bank-specific loan type breakdown</span> — real estate heavy banks behave differently from consumer credit banks</li><li>Turn the regression coefficients into a <span className={styles.reflectHL}>simple scoring tool</span>, given a bank's size, last year's NPL, and current macro data, it could output a risk trajectory estimate without needing a full model rerun</li></ul></div> 
                                  </div>
                                </div>
